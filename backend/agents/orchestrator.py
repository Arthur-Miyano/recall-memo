...
class SessionState(str, Enum):
    """会话状态机，见文档 3.3。面试/回忆模式本阶段仅预留状态，不实现流程。"""

    IDLE = "IDLE"
    # 记忆训练模式
    MEMORIZE_SHOW = "MEMORIZE_SHOW"  # 展示题干+答案供记忆
    MEMORIZE_QUIZ = "MEMORIZE_QUIZ"  # 打乱顺序考核中
    # 面试模拟模式（Phase 3 实现，预留）
    INTERVIEW_SELECT = "INTERVIEW_SELECT"
...
class OrchestratorAgent(BaseAgent):
    ...
    async def create_session(
        ...
        """开始会话：策略 Agent 抽题 → 进入 MEMORIZE_SHOW，返回题干+答案供记忆。"""
        if mode != "memorize":
            raise StateError(f"模式 {mode} 本阶段未实现（面试/回忆模式仅预留状态）")
        ...
        # 抽题完成，进入记忆展示阶段
        self._transition(db, session, SessionState.MEMORIZE_SHOW, self.name)
        return {
    ...
    async def start_quiz(self, db: DBSession, session_id: int) -> dict[str, Any]:
        """用户确认记好了：打乱顺序，进入 MEMORIZE_QUIZ，返回第一题变体题干。"""
        session = self._get_session(db, session_id)
        if session.state != SessionState.MEMORIZE_SHOW.value:
            raise StateError(f"当前状态 {session.state} 不能开始考核（需在 MEMORIZE_SHOW 状态）")

        quiz_order = list(session.question_ids)
        random.shuffle(quiz_order)
        session.quiz_order = quiz_order
        session.current_index = 0
        session.context = {"variants": {}, "results": []}
        db.add(session)
        db.commit()

        question = db.get(Question, quiz_order[0])
        # 面试官 Agent 生成第一题变体题干
        self._transition(db, session, SessionState.MEMORIZE_QUIZ, self.interviewer.name)
    ...
    async def get_current(self, db: DBSession, session_id: int) -> dict[str, Any]:
        """当前题：变体题干 + 关键词（半开卷提示用），不含答案。"""
        session = self._get_session(db, session_id)
        if session.state != SessionState.MEMORIZE_QUIZ.value:
    ...
    async def submit_answer(self, db: DBSession, session_id: int, answer: str) -> dict[str, Any]:
        """提交回答：评分 Agent 与智能助理写库并行，返回评分并推进到下一题。"""
        session = self._get_session(db, session_id)
        if session.state != SessionState.MEMORIZE_QUIZ.value:
        ...
        self._transition(db, session, SessionState.MEMORIZE_QUIZ, self.grader.name)
        ...
        # 记录本题结果到会话上下文（整体重新赋值触发 JSON 列更新）
        ...
        if session.current_index < len(session.quiz_order):
            # 还有下一题：面试官生成新变体，继续 MEMORIZE_QUIZ
    ...
    def _store_variant(self, db: DBSession, session: Session, question_id: int, variant: str) -> None:
        ...
        session.context = {**session.context, "variants": variants}
        db.add(session)
        db.commit()
        db.refresh(session)
    ...
    def _build_summary(session: Session) -> dict[str, Any]:
        ...
        return {
            ...
            "reciting_count": sum(1 for r in results if r.get("score", {}).get("is_reciting")),
            "per_question": [
