...
选好技术栈和题量就可以开始。技术栈支持多选（比如 Python + Agent 一起背），点「混合」则不限栈。

![记忆训练](docs/screenshots/02-memorize.png)

记忆训练先给题干和标准答案，确认记好了以后打乱顺序开考，题干会换成面试官口吻的说法。
题量不固定，想背几道自己填，不限制 3 / 5 / 7 这种档位。

看答案的时候左边还有一棵"记忆树"：AI 把答案拆成层级大纲，只理结构不改原文，
一长段文字变成可以逐级展开的关键词树，先记骨架再填细节，比硬背一大段好记得多。
想专心背原文也可以把树收起来，或者放大看。
...
另外给出薄弱点分析和复习建议。答案旁边同样配了记忆树，对着树再过一遍印象更深。

![题库总览](docs/screenshots/03-bank.png)

题库按技术栈和知识点两级分组，每个知识点一格，背过就涂黑。
内置 229 题（Agent / 数据库 / Python 为主，另有少量 Java / Vue3 / 算法等），也可以导入自己的资料。
...
它会自动起服务并打开浏览器，前后端一个进程搞定。桌面快捷方式是个水墨风的小螃蟹图标。
端口不固定：8000 被其他项目占用时会自动顺移到 8001、8002……，等自己的服务就绪后才打开浏览器，
不会误开别人占着 8000 的页面；已经有一个实例在跑时，双击只会重新打开它，不会重复启动。启动过程有单实例锁，
连续点击不会抢同一个端口；启动失败时不会再打开无效网页，会显示 `logs/launcher.log` 或对应的 `logs/backend-端口.log`。
...
- 后端：FastAPI + SQLModel（SQLite），Agent 分工协作（总控 / 面试官 / 评分 / 策略 / 助理 / 录入清洗 / 记忆树生成）
...
后端测试：`cd backend && pytest tests/ -q`（483 项，带覆盖率门禁：生产代码行覆盖率 ≥ 90%）

> 测试临时目录（`tmp_path`）已配置自动清理（`tmp_path_retention_policy = none`），
> 每次运行结束后自动删除，不会堆积 `.pytest-tmp-*` 目录。

前端测试：`cd frontend && npm test`（60 项：保存队列、面试倒计时、记忆树折叠状态等行为回归测试）
