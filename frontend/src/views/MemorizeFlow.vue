...
<template>
  <section class="screen active">
    ...
    <!-- 出题加载遮罩：半透明黑罩住整页，卡片居中，等待期间挡住误操作 -->
    <div v-if="busy === '面试官 AGENT 出题中…'" class="mem-load-overlay">
      <div class="paper mem-load-card" role="status" aria-live="polite">
        <div class="paper-head">
          <span class="no">AGENT</span>
          <h3>面试官正在出题中…</h3>
        </div>
        <p class="comment">正在结合本轮题目生成考核问法，通常需要几秒钟，请稍候。</p>
        <div class="pixelbar"><i></i></div>
      </div>
    </div>
